...
import com.power.authority.authorization.exception.CustomException;
...
public class JedisUtils {
    ...
    public static Object getObject(String key) {
        ...
        try {
            ...
            if( bytes!=null && bytes.length>0) {
                return SerializeUtils.unserializable(bytes);
            }
        } catch (Exception e) {
    ...
    public static String setObject(String key, Object value) {
        ...
        try {
            jedis = jedisPool.getResource();
            return jedis.set(key.getBytes(), (byte[]) SerializeUtils.serializable(value));
        } catch (Exception e) {
    ...
    public static String setObject(String key, Object value, int expiretime) {
        ...
        try {
            jedis = jedisPool.getResource();
            result = jedis.set(key.getBytes(), (byte[]) SerializeUtils.serializable(value));
            if(OK.equals(result)) {
     ...
     * @param minus
     */
    public void setExpire(String key,int minus){
        Jedis client = getJedis();
        try{
            client.expire(key,minus);
        }catch (Exception e){
            e.printStackTrace();
            client.close();
        }finally {
     ...
     * @param minus
     */
    public void setPExpire(String key,long minus){
        Jedis client = getJedis();
        try{
            client.pexpire(key,minus);
        }catch (Exception e){
            e.printStackTrace();
            client.close();
        }finally {
    ...
    public <T extends Serializable> void setList(String key , List<T> list){
        Jedis client = getJedis();
        try {
            ObjectTranscoder<T> objectTranscoder = new ObjectTranscoder<T>();
            client.set(key.getBytes(),objectTranscoder.serialize(list));
        } catch (Exception e) {
    ...
    public <T extends Serializable> List<T> getList(String key){
        Jedis client = getJedis();
        byte[] in = client.get(key.getBytes());
        ObjectTranscoder<T> objectTranscoder = new ObjectTranscoder<T>();
